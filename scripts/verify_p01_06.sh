...
go vet ./kernel/...
go test ./kernel/...
go test -race ./kernel/internal/storage -run 'Test(LoadConfiguration|SettingsFromConfig|RenderKey|ValidateUpload|IntegrityReader|VerifiedReadCloser|ProviderFailure|FailureClassification|ObjectInfo|DownloadStream|StorageConnectionVerificationTimeout|StorageOperationTimeout)' -count=1
go test -v ./kernel/internal/storage -run 'TestUnavailableStorageConnectionIsBoundedAndSafe|TestS3CompatibleStorageFoundationIntegration|TestS3CompatibleStoreSupportsConcurrentDistinctObjects' -count=1
...
if grep -R -nE 'type[[:space:]]+(Tenant|User|Customer|Order|Invoice|Product|Agent|Planner|Plan|Model|Embedding|Vector|Workflow|Event)([[:space:]]|$)|[[:space:]](TenantID|UserID|AgentID|GoalID|PlanID|TaskID|ExecutionID)[[:space:]]' kernel/internal/storage --include='*.go'; then
  echo "ERROR: P01.06 storage source declares unauthorized tenant/business/AI/workflow concepts" >&2
  exit 1
fi
...
echo "P01.06 G2 unit/race key/metadata/stream/integrity/classification: PASS"
echo "P01.06 G3 S3-compatible provider contract/integration/concurrency: PASS"
